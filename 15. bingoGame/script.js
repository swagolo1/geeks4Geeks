...
function createBingoCard() { 
	...
	while (usedNumbers.size < ROWS * COLS) { 
		...
		if (!usedNumbers.has(num)) usedNumbers.add(num);
	} 
...
document 
	...
	.addEventListener('click', () => { 
		...
		document.getElementById('numberInput').disabled = false; 
		document.getElementById('turnDisplay').textContent = 'Player 1\'s Turn'; 
		document.getElementById('winDisplay').textContent = ''; // Clear win message 
	}); 

//marking the number entered
